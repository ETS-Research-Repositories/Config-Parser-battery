...
from yaml_parser.utils import dict_merge
from yaml_parser.yaml_parser import yaml_load, YAMLArgParser


class ConfigManger:

    def __init__(
            self, DEFAULT_CONFIG_PATH: str = None, verbose=True
    ) -> None:
        ...
        if self._default_path:
            self._default_config = yaml_load(
                self._default_path, verbose=False
            )

        self._merged_config = dict_merge(
            self._default_config or {}, self._parsed_args
        )
        if verbose:
